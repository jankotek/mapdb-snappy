 ...
 * load order by setting the 'mapdb.snappy' system property.  Example:
 *
 * <code>
...
public class Snappy {
    ...
    static {
        SPI attempt = null;
        String[] factories = System.getProperty("mapdb.snappy", "iq80,xerial").split(",");
        for (int i = 0; i < factories.length && attempt==null; i++) {
            String name = factories[i];
            try {
                name = name.trim();
                if("xerial".equals(name.toLowerCase())) {
                    name = Snappy.class.getName()+"$XerialSnappy";
                } else if("iq80".equals(name.toLowerCase())) {
                    name = Snappy.class.getName()+"$IQ80Snappy";
                }
